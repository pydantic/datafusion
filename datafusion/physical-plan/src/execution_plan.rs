...
pub trait ExecutionPlan: Debug + DisplayAs + Send + Sync {
    ...
    fn with_filter_pushdown_result(
        self: Arc<Self>,
        _pushdown: &[FilterPushdownResult]
    ) -> Result<Option<Arc<dyn ExecutionPlan>>> {
        Ok(None)
    }
    ...
    ) -> Result<Option<FilterPushdownResult>> {
        Ok(None)
    }
...
pub struct FilterPushdownResult {
    /// The new execution plan after the filter pushdown
    pub plan: Arc<dyn ExecutionPlan>,
    /// The filter pushdown result for each filter
    pub pushdown: Vec<FilterPushdownSupport>,
}

pub enum FilterPushdownSupport {
    /// A node such as an aggregation that does not accept filter pushdown.
    Inexact,
    /// A node such as a repartition that accepts filter pushdown.
    /// This includes a new execution plan and a filter pushdown result for each filter.
    Exact,
}
