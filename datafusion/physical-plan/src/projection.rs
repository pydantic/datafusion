...
impl ProjectionExec {
    ...
    /// Builds a `ProjectionExec` from `projector` and `input`.
    ///
    /// `reused_eq_group`, when present, is used as the output equivalence group
    /// instead of projecting the input's group again.
    ///
    ...
    fn compute_properties(
        ...
        // The only thing reuse changes: the projected equivalence group is
        // handed back rather than recomputed. Everything below is common, so
