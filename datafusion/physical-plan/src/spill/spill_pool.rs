...
mod tests {
    ...
    type ReadDelay = Arc<dyn Fn() -> Duration + Send + Sync>;

    /// Test double for a spill file that runs `write_hook` before every disk
    /// write or flush, and delays the first bytes of every read stream by
    /// `read_delay()`. Writers write while holding their file's lock, so a hook
    /// that blocks pauses a `push_batch` at exactly the point where it has a
    /// file checked out for writing; a read delay makes an older file's bytes
    /// arrive after a newer file's. Data still goes to real temporary files.
    ...
    impl TempFileFactory for IoHookFactory {
        fn create_temp_file(&self, description: &str) -> Result<Arc<dyn SpillFile>> {
            Ok(Arc::new(IoHookFile {
                ...
                read_delay: Arc::clone(&self.read_delay),
            }))
        }
    }
    ...
    /// delay reads by `read_delay()`, plus the `DiskManager` that owns the files
    /// so tests can check disk usage.
    ...
    -> Result<()> {
        ...
        let (spill_manager, _disk_manager) =
            spill_manager_with_io_hooks(write_hook, Arc::new(|| Duration::ZERO))?;
    ...
    async fn run_spill_pool_scenario(seed: u64) -> Result<()> {
        ...
        let (spill_manager, disk_manager) =
            spill_manager_with_io_hooks(write_hook, read_delay)?;
