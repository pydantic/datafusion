...
fn roundtrip_parquet_exec_with_pruning_predicate() -> Result<()> {
    ...
    let file_source = ParquetSource::new(options, scan_config).with_predicate(predicate);

    roundtrip_test(DataSourceExec::from_data_source(file_source))
}
...
async fn roundtrip_parquet_exec_with_table_partition_cols() -> Result<()> {
    ...
    let file_source = ParquetSource::new(TableParquetOptions::default(), scan_config);
...
async fn roundtrip_projection_source() -> Result<()> {
    ...
    let filter = Arc::new(
        FilterExec::try_new(
            ...
            Arc::new(DataSourceExec::new(file_source.as_data_source())),
        )?
        .with_projection(Some(vec![0, 1]))?,
    );
