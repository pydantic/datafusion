...
fn roundtrip_parquet_exec_with_pruning_predicate() -> Result<()> {
    ...
    roundtrip_test(DataSourceExec::from_data_source(
        ParquetSource::new(scan_config, options).with_predicate(predicate),
    ))
}
...
async fn roundtrip_parquet_exec_with_table_partition_cols() -> Result<()> {
    ...
    roundtrip_test(DataSourceExec::from_data_source(ParquetSource::new(
        scan_config,
...
fn roundtrip_parquet_exec_with_custom_predicate_expr() -> Result<()> {
    ...
    let file_source = ParquetSource::new(scan_config, TableParquetOptions::default())
    ...
    let exec_plan = DataSourceExec::from_data_source(file_source);
...
async fn roundtrip_projection_source() -> Result<()> {
    ...
    let file_source = ParquetSource::new(scan_config, TableParquetOptions::default())
        .with_projected_statistics(statistics);
    ...
    let filter = Arc::new(
        FilterExec::try_new(
            ...
            DataSourceExec::from_data_source(parquet_source),
        )?
        .with_projection(Some(vec![0, 1]))?,
    );
