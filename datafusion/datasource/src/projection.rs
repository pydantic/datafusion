    ...
    fn open(&self, partitioned_file: PartitionedFile) -> Result<FileOpenFuture> {
        ...
        Ok(async move {
            ...
            let stream = stream.map(move |batch| {
                let batch = batch?;
                // TODO: pre-compute projected_schema before opening files
                let projected_schema =
                    Arc::new(projection.project_schema(&batch.schema())?);
                let batch =
                    projection.project_batch(&batch, Arc::clone(&projected_schema))?;
                Ok(batch)
            });
            Ok(stream.boxed())
        }
...
mod test {
    ...
    fn test_split_projection_with_partition_columns() {
        ...
        let output_schema = Arc::new(
            injected_projection
                .project_schema(&file_batch.schema())
                .unwrap(),
        );
        let result = injected_projection
            .project_batch(&file_batch, output_schema)
            .unwrap();
    ...
    fn test_inject_partition_columns_multiple_partitions() {
        ...
        // Apply projection
        let output_schema = Arc::new(injected.project_schema(&data.schema()).unwrap());
        let result = injected.project_batch(&data, output_schema).unwrap();
