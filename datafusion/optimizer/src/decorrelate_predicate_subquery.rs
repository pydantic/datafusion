    ...
    fn rewrite(
        ...
        match plan {
            ...
            LogicalPlan::Projection(projection) => {
                ...
                // Only decorrelate when a single projection expression carries
                // predicate subqueries. Spreading them across multiple output
                // columns stacks several `LeftMark` joins whose mark columns
                // collapse to the same unqualified `mark` name once later passes
                // (e.g. `optimize_projections`) prune intermediate qualifiers,
                // which fails schema validation. Bail out and leave the plan
                // unchanged in that case so behavior matches the pre-existing
                // (un-decorrelated) path rather than emitting an invalid plan.
                // Multiple subqueries within one expression (e.g. a single CASE)
                // are still handled below.
                if projection.expr.iter().filter(|e| has_subquery(e)).count() > 1 {
                    return Ok(Transformed::no(LogicalPlan::Projection(projection)));
                }
