...
impl PredicateRewriter {
    ...
    /// Translate logical filter expression into pruning predicate
    /// 
    /// See [`PredicateRewriter::rewrite_predicate_to_statistics_predicate`] for more details.
    ///
    /// This function also takes a mutable reference to a `RequiredColumns` struct that will be
    /// populated with the columns that are required to evaluate the pruning predicate.
    pub fn rewrite_predicate_to_statistics_predicate_with_required_columns(
        &self,
        expr: &Arc<dyn PhysicalExpr>,
        schema: &Schema,
        required_columns: &mut RequiredColumns,
