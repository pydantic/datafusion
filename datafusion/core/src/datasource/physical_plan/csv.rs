...
mod tests {
    ...
    async fn csv_exec_with_projection(
        ...
        let config =
            FileScanConfigBuilder::from(partitioned_csv_config(file_schema, file_groups))
        ...
        let source = CsvSource::new(true, b',', b'"', config);

        let csv = DataSourceExec::from_data_source(source);
