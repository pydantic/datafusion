...
impl SessionState {
    ...
    /// The resolved built-in dialect (which is owned) is stored in `fallback` so
    /// the returned reference can borrow from it.
    #[cfg(feature = "sql")]
    fn resolve_sql_dialect<'a>(
        &'a self,
        dialect: &Dialect,
        fallback: &'a mut Option<Box<dyn SqlParserDialect>>,
    ) -> datafusion_common::Result<&'a dyn SqlParserDialect> {
        if let Some(dialect) = &self.sql_dialect {
            return Ok(dialect.as_ref());
        }
    ...
    pub fn sql_to_statement(
        ...
        let dialect = self.resolve_sql_dialect(dialect, &mut fallback)?;
