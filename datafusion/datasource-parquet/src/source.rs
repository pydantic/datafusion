...
/// let parquet_source = data_source.as_any().downcast_ref::<ParquetSource>().unwrap();
/// let base_config = parquet_source.config();
...
///       .build();
///
///     let parquet_source = parquet_source.clone().with_config(new_config);
///
///     (Arc::new(DataSourceExec::new(parquet_source)))
///   })
///   .collect::<Vec<_>>();
/// ```
///
...
/// let config = FileScanConfigBuilder::new(ObjectStoreUrl::local_filesystem(), schema(), Arc::new(ParquetSource::new(TableParquetOptions::default())))
...
impl ParquetSource {
    ...
    pub fn new(
        ...
        Self {
            ...
            #[cfg(feature = "parquet_encryption")]
            encryption_factory: Default::default(),
            config,
        }
    }
    ...
    pub fn apply_schema_adapter(self) -> datafusion_common::Result<Arc<dyn FileSource>> {
        let file_source: Arc<dyn FileSource> = self.clone().into();

        // If the FileScanConfig.file_source() has a schema adapter factory, apply it
        if let Some(factory) = self.config.schema_adapter_factory {
...
impl FileSource for ParquetSource {
    ...
    fn create_file_opener(
        ...
        let (expr_adapter_factory, schema_adapter_factory) = match (
            ...
            base_config.schema_adapter_factory.as_ref(),
        ) {
    ...
    fn projected_statistics(&self) -> Statistics {
        let statistics = self.config().projected_stats();
