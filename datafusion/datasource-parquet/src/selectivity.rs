        ...
        for (id, expr) in filters {
            ...
            let Some(state) = state else {
                ...
                if byte_ratio <= config.byte_ratio_threshold {
                    debug!(
                        "FilterId {id}: New filter → Row filter via byte ratio {} <= threshold {} — {expr}",
                    ...
                    debug!(
                        "FilterId {id}: New filter → Post-scan via byte ratio {} > threshold {} — {expr}",
                        byte_ratio, config.byte_ratio_threshold
                    );
                    self.filter_states.insert(id, FilterState::PostScan);
            ...
            match state {
                FilterState::RowFilter => {
                    ...
                    {
                        debug!(
                            "FilterId {id}: Row filter → Post-scan via CI upper bound < {} bytes/sec — {expr}",
