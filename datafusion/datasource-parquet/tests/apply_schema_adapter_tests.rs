...
mod parquet_adapter_tests {
    ...
    use datafusion_common::{
        config::TableParquetOptions, ColumnStatistics, DataFusionError, Result,
    };
    ...
    fn test_apply_schema_adapter_with_factory() {
        ...
        let source = source.as_any().downcast_ref::<ParquetSource>().unwrap();

        // Apply schema adapter to a new source
        let result_source = source.clone().apply_schema_adapter().unwrap();
