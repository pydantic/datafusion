...
### `get_coerce_type_for_list` and `get_coerce_type_for_case_when` are deprecated

`IN` and `CASE expr WHEN` are defined in terms of `=`, and `=` now reads a
timezone-naive timestamp in `datafusion.execution.time_zone` when it is compared
against a timezone-aware one. These two functions have nowhere to take that
setting from, so the planner no longer calls them and they now return a coercion
that disagrees with the comparison operator they are defined in terms of.

Both remain in place and behave exactly as they did in 55.0.0. To follow the
planner, fold `comparison_coercion_with_session_timezone` over the values
yourself:

```rust
// 55.0.0
let coerced = get_coerce_type_for_list(&expr_type, &list_types);
...
Pass `options.execution.time_zone` as `session_time_zone`, or `None` where no
session is in scope — `None` reproduces the 55.0.0 behavior, except that `NULL`
against a dictionary-encoded type coerces to the dictionary type rather than to
its value type, which is what `=` has always done.
