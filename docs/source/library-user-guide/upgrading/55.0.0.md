...
### `parse_table_schema_from_proto` is deprecated

`datafusion_proto::physical_plan::from_proto::parse_table_schema_from_proto` is
deprecated in favor of the equivalent method on `FileScanConfig`, which lives in
`datafusion-datasource` alongside the rest of the shared file-scan protobuf
conversions:
...
`PartitionedFile`, `FileGroup` and `FileRange` also gained inherent
`try_to_proto` / `try_from_proto` methods (in `datafusion_datasource::proto`,
behind the `proto` feature) which now own that wire logic;
`datafusion-proto`'s `TryFromProto` implementations for those types delegate to
