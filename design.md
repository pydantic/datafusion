...
### 8.4 Single-row-group files and partition skew

When `lineitem` is one file of one row group, the scheduler's runtime samples cause it to auto-demote unhelpful in-scan filters to post-scan very quickly, and the resulting plan looks like `FilterExec` above `RepartitionExec`-style execution — which is exactly the plan shape on `main + pushdown=off`. This is why TPC-H SSD lands at 0.89× of `main`, not just at parity: the byte-ratio prior wants row-level, runtime samples say "no payoff", and the demote-to-post-scan path lets the existing shuffle do its job. The "real" fix for partition skew remains row-group morselization at [#21766](https://github.com/apache/datafusion/pull/21766) — but the lower bound this design delivers is already below `main`.
